    ...
    async.series(
      [
        async function () {
          ...
          // Analyze URL image
          console.log(
            "Analyzing type in image...",
            typeURLImage.split("/").pop()
          );
          const types = (
            await computerVisionClient.analyzeImage(typeURLImage, {
          ...
          function describeType(imageType) {
            if (
              ...
              imageType.clipArtType > imageType.lineDrawingType
            )
              return "clip art";
            if (
              imageType.lineDrawingType &&
              imageType.clipArtType < imageType.lineDrawingType
            )
          ...
          // Analyze URL image
          console.log(
            "Analyzing category in image...",
            categoryURLImage.split("/").pop()
          );
          const categories = (
            await computerVisionClient.analyzeImage(categoryURLImage)
          ).categories;
          ...
          function formatCategories(categories) {
            ...
            return categories
              .map((cat) => `${cat.name} (${cat.score.toFixed(2)})`)
              .join(", ");
          }
          // </snippet_categories_format>
          /**
          ...
          // Analyze URL image
          console.log(
            "Analyzing brands in image...",
            brandURLImage.split("/").pop()
          );
          const brands = (
          ...
          // Analyze URL image
          console.log(
            "Analyzing image for color scheme...",
            colorURLImage.split("/").pop()
          );
          console.log();
          const color = (
            await computerVisionClient.analyzeImage(colorURLImage, {
          ...
          function printColorScheme(colors) {
            console.log(
              `Image is in ${colors.isBwImg ? "black and white" : "color"}`
            );
            console.log(`Dominant colors: ${colors.dominantColors.join(", ")}`);
            console.log(
              `Dominant foreground color: ${colors.dominantColorForeground}`
            );
            console.log(
              `Dominant background color: ${colors.dominantColorBackground}`
            );
          ...
          await computerVisionClient
            .generateThumbnail(100, 100, dogURL, { smartCropping: true })
            .then((thumbResponse) => {
          ...
          const domain = (
            await computerVisionClient.analyzeImageByDomain(
              "landmarks",
              domainURLImage
            )
          ).result.landmarks;
          ...
          if (domain.length) {
            console.log(
              `${domain.length} ${
                domain.length == 1 ? "landmark" : "landmarks"
              } found:`
            );
          ...
          function formatRectDomain(rect) {
            if (!rect) return "";
            return (
              `top=${rect.top}`.padEnd(10) +
              `left=${rect.left}`.padEnd(10) +
